...
class SoftMaxCrossEntropy:
    ...
    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        return pred,self._cross_entropy(y,pred)
...
class Linear:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: perform forward pass and save any values you may need for
        #  the backward pass
        self.cache = np.concatenate((np.array([1]),x))
        return np.dot(self.weight,self.cache)
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        self.dw = np.outer(dz,self.cache)
        return np.dot(self.weight[:,1:].T,dz)
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        pred_y = self.softmax._softmax(b)
        j = self.softmax._cross_entropy(y,pred_y)
        return pred_y,j
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        dj = 1
        ds = self.softmax.backward(y,y_hat)
        db = self.linear2.backward(ds)
        dz = self.sigmoid.backward(db)
        da = self.linear1.backward(dz)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for i in range(n_epochs):
            shuff_x,shuff_y = shuffle(X_tr,y_tr,i)

            for x,y in zip(shuff_x,shuff_y):
                self.backward(y,self.forward(x,y)[0])
                self.step()
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        for i in range(len(X)):
            label = np.argmax(self.forward(X[i],y)[0])
            labels.append(label)
            if label != y[i]:
                errors+=1
        return labels,errors/len(y)
...
if __name__ == "__main__":
    ...
    # train_losses_5, test_losses_5 = nn_5.train(X_tr, y_tr, X_test, y_test, n_epochs)
    # train_losses_20, test_losses_20 = nn_20.train(X_tr, y_tr, X_test, y_test, n_epochs)
    # train_losses_50, test_losses_50 = nn_50.train(X_tr, y_tr, X_test, y_test, n_epochs)
    # train_losses_100, test_losses_100 = nn_100.train(X_tr, y_tr, X_test, y_test, n_epochs)
    # train_losses_200, test_losses_200 = nn_200.train(X_tr, y_tr, X_test, y_test, n_epochs)
    # train_list = [train_losses_5[-1],train_losses_20[-1],train_losses_50[-1],train_losses_100[-1],train_losses_200[-1]]
    # test_list = [test_losses_5[-1],test_losses_20[-1],test_losses_50[-1],test_losses_100[-1],test_losses_200[-1]]
    # xl = [5,20,50,100,200]
